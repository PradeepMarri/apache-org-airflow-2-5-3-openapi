...
func Get_datasetsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if val, ok := args["uri_pattern"]; ok {
			queryParams = append(queryParams, fmt.Sprintf("uri_pattern=%v", val))
		}
		...
		url := fmt.Sprintf("%s/datasets%s", cfg.BaseURL, queryString)
		...
		var result models.DatasetCollection
...
func CreateGet_datasetsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_datasets",
		...
		mcp.WithString("uri_pattern", mcp.Description("If set, only return datasets with uris matching this pattern.\n")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Get_datasetsHandler(cfg),
	}
}
