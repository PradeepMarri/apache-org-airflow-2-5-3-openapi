...
func Get_logHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/dags/%s/dagRuns/%s/taskInstances/%s/logs/%s%s", cfg.BaseURL, queryString)
...
func CreateGet_logTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_dags_dag_id_dagRuns_dag_run_id_taskInstances_task_id_logs_task_try_number",
		mcp.WithDescription("Get logs"),
	)
